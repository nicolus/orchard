...
# Path to the custom $(hostname) Root CA certificate.
PATH_ROOT_CNF="${PATH_SSL}/ca.orchard.$(hostname).cnf"
PATH_ROOT_CRT="${PATH_SSL}/ca.orchard.$(hostname).crt"
PATH_ROOT_KEY="${PATH_SSL}/ca.orchard.$(hostname).key"
...
BASE_CNF="
    [ ca ]
    default_ca = ca_orchard_$(hostname)

    [ ca_orchard_$(hostname) ]
    dir           = $PATH_SSL
    certs         = $PATH_SSL
